    ...
    public void ProcessRequest (HttpContext context) {
        ...
        switch (_sAttachmentID)
        {
            ...
            case "BusinessVasDefaultSet":
                ...
                break;
            case "ProgramRebateMapping":
                if (context.Session["ProgramRebateMappingSql"] != null)
                {
                    string _sQuery = context.Session["ProgramRebateMappingSql"].ToString();
                    DataSet _oDataSet = SYSConn<MSSQLConnect>.Connect().GetDS(_sQuery);
                    WebFunc.ExportDataSetToExcelAndZip(_oDataSet, "ProgramRebateMapping", false, Encoding.UTF8, WebFunc.ExportContentType.Excel, true, true, false);
                }
                break;
        }
    }
