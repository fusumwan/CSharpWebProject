...
function c_all_opt() {
    ...
    */
    if (document.getElementById('m_card_exp_year') != undefined) document.getElementById('form1').m_card_exp_month.value = ""
    if (document.getElementById('extra_d_charge') != undefined) document.getElementById('form1').m_card_exp_year.value = ""
...
function chk_save(ThisForm) {
    ...
    if (bFlag) {
        ...
        if ($("#program").val() == "IPHONE REBATE PLAN" || $("#program").val() == "STAFF OFFER (IPHONE REBATE)" || $("#program").val() == "IPAD SIM ONLY OFFER" || $("#program").val() == "MOBILE LITE (SIM ONLY)" | $("#program").val() == "MOBILE LITE (HS OFFER)" || $("#program").val() == "STAFF OFFER (IPHONE CONCIERGE SER)" || $("#program").val() == "IPHONE CONCIERGE SERVICE" || $("#program").val() == "MOBILEONE T3 RETENTION") {
            /*if ($("#sim_item_name1").val() == '') {
            alert("Please select a SIM Card Name!");
            bFlag = false;
            } else if ($("#sim_item_code").val() == '') {
            alert("Please re-select a SIM Card Name!");
            bFlag = false;
            } else {*/
            /*
            if ($("#sim_item_number").val() == '') {
            PreviousOrderModifyNoCheck.check_sim_number_for_js($("#sim_item_name1").val(), $("#sim_item_code").val(), $("#sim_item_number").val(), callback_check_sim_number_for_js);
            } else {
            PreviousOrderModifyNoCheck.check_sim_number_for_js($("#sim_item_name1").val(), $("#sim_item_code").val(), $("#sim_item_number").val(), callback_check_sim_number_for_js);
            }
            */
    ...
    if (bFlag) {
        try {
            if (!CheckDeliveryDateTime()) {
                //Modify With Out Checking No Need To Check Delivery Date
                //event.returnValue = false;
                //bFlag = false;
            }
        } //113
    ...
    /*
    //ModifyNoWithOutChecking No Need to Check.
    if (bFlag) {
        try {
            if (!ChkUpgradeOrder()) {
            event.returnValue = false;
    ...
    */
    ...
    /*
    //Modify With Out Check Shouldn't need to check Customer Name
    if (bFlag) {
    try {
    if (!ChkCustomerName()) {
    event.returnValue = false;
    ...
    */
    ...
    var PassCheckingObj = document.getElementById('PassChecking');
    if (PassCheckingObj != undefined) {
        if (PassCheckingObj.checked == true) {
