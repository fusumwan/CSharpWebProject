...
namespace NEURON.ENTITY.FRAMEWORK
{
    public class FuncHelper
    {
        ...
        public static bool IsParseDateTime(string xObj,string x_sDateTimeFormat)
        {
            if (xObj == null) return false;
            try
            {
                if (string.IsNullOrEmpty(xObj)) { return false; }

                DateTime _dValue;
                if (DateTime.TryParseExact(xObj.Trim(), x_sDateTimeFormat, CultureInfo.InvariantCulture, DateTimeStyles.AllowWhiteSpaces, out _dValue))
                {
                    return true;
                }
                else
        ...
        public static DateTime ConvertDatetime(string x_sDate, string[] x_oPattern)
        {
            return DateTime.ParseExact(x_sDate, x_oPattern, CultureInfo.InvariantCulture, DateTimeStyles.AllowWhiteSpaces);
        }
        #endregion
        ...
        #region Find Control
        /// <summary>
        /// Find any control on page using recursion. This could be in some Util/Helper class.
        /// </summary>
        /// <param name="page"></param>
        /// <param name="controlId"></param>
        /// <returns></returns>
        public static Control FindAnyControl(Page x_oPage, string x_sControlId) {
            return FindControlRecursive(x_sControlId, x_oPage.Form); 
        }      
        public static Control FindAnyControl(UserControl x_oControl, string x_sControlId) {
            return FindControlRecursive(x_sControlId, x_oControl); 
        }
        public static Control FindControlRecursive(string _sControlId, Control x_oParent)
        {
            foreach (Control _oControl in x_oParent.Controls)
            {
                if (_oControl.ID.Equals(_sControlId)) { return _oControl; }
                Control _oResult = FindControlRecursive(_sControlId, _oControl);
                if (_oResult != null) { return _oResult; } 
            }
            return x_oParent.FindControl(_sControlId); 
        } 
        #endregion
    }
